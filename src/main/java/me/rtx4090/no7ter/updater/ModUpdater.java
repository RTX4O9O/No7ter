package me.rtx4090.no7ter.updater;
...
public class ModUpdater {
    ...
    private static void checkForUpdate() throws ApiException, IOException {
        ...
        final String GITHUB_URL = "https://discordapp.com/channels/1167573652024340490/1167575369675718697";
        ChatUtil.addChatMessage(new ChatComponentText(ChatUtil.getTagMW() + EnumChatFormatting.DARK_PURPLE + "Phantom "
                + EnumChatFormatting.DARK_GRAY + "version " + latestVersion + EnumChatFormatting.LIGHT_PURPLE + " is available, click this message to see the changelog and download page.")
                .setChatStyle(new ChatStyle()
                        .setChatClickEvent(new ClickEvent(ClickEvent.Action.OPEN_URL, GITHUB_URL))
                        .setChatHoverEvent(new HoverEvent(HoverEvent.Action.SHOW_TEXT, new ChatComponentText(EnumChatFormatting.DARK_PURPLE + GITHUB_URL)))));
        ...
        if (modCacheFile.exists() && deleterFile.exists()) {
            ...
            ChatUtil.addChatMessage(ChatUtil.getTagMW() + EnumChatFormatting.DARK_PURPLE + "Phantom "
                    + EnumChatFormatting.DARK_GRAY + "version " + latestVersion
                    + EnumChatFormatting.LIGHT_PURPLE + " has been downloaded and will be installed to your mods folder automatically when closing your game.");
